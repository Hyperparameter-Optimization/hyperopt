...
def submission_creation(path_to_train, path_to_test, hyperparameters, outfile):
    dtrain, trainvars = create_dtrain(path_to_train)
    model = create_model(hyperparameters, dtrain)
    threshold = find_threshold(path_to_train, hyperparameters)
    thresholded_model = create_model(
        hyperparameters, dtrain, ams_threshold=threshold)
    print('Found threshold is: ' + str(threshold))
    evaluate_test(thresholded_model, path_to_test, trainvars, outfile, threshold)


def find_threshold(path_to_train, hyperparameters):
    train_df, trainvars = load_df(path_to_train)
    threshold = kfold_threshold(train_df, trainvars, hyperparameters)
    return threshold


def kfold_threshold(prepared_data, trainvars, hyperparameters):
    ...
    for train_index, test_index in kfold.split(prepared_data):
        ...
        threshold, score = evaluate(dtrain, dtest, hyperparameters)
...
def create_model(hyperparameters, dtrain, ams_threshold=0.15):
